...
def post_mail(title,mail_content,send_mail):
    ...
    while success_num < 5:
        try:
            ...
            res = s.post(
                url="http://127.0.0.1:5000/mail",  # HTML文本翻译
...
def post_google_translate(language,source):
    ...
    s.mount('https://', HTTPAdapter(max_retries=5))
    res = s.post(
        url="http://127.0.0.1:5000/translate-google",  # 翻译
        data=post_data,timeout=120,)
...
def post_caiyun_translate(language,source):
    ...
    s.mount('https://', HTTPAdapter(max_retries=5))
    res = s.post(
        url="http://127.0.0.1:5000/translate-caiyun",  # 翻译
        data=post_data,timeout=120,)
...
def test():
    ...
    <p class="cont">本文为自媒体、作者等在百度知道日报上传并发布，仅代表作者观点，不代表百度知道日报的观点或立场，知道日报仅提供信息发布平台。合作及供稿请联系zdribao@baidu.com。</p>
    </div>
    </div>'''
